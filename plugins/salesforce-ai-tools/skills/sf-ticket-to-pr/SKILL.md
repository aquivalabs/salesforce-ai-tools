...
Do not retrieve metadata after a successful deploy just to inspect org-normalized
XML. Retrieve only when source shape is unknown or a deploy error requires it.
...
### Checkpoint before UI evidence

After changed files deploy, tests pass, and code analysis is clean, commit and
push the implementation before spending time on Playwright evidence:

    git add <changed source files and knowledge files>
    git commit -m "<one short sentence>"
    git push

Do not include incomplete `.verification/` files in this checkpoint. This keeps
working code on the branch if video capture, frame inspection, or Salesforce UI
loading consumes the remaining job budget. After the checkpoint, continue with
Playwright and amend with evidence only if it completes cleanly.
...
Try at most one focused re-record after an otherwise successful UI run. If the
backend state, page context, and at least one screenshot prove the core behavior
but dynamic Lightning content does not refresh into the final frame, stop and
ship with `UI-INCONCLUSIVE` plus the SOQL/test evidence. Do not keep polishing
video evidence after the implementation is known-good.

## Step 4 — Ship

If you already pushed the checkpoint, add evidence in a small follow-up commit.
Do not amend a pushed checkpoint. Otherwise commit and push now:

    git add <changed files and verified evidence>
    git commit -m "<one short sentence>"
    git push
